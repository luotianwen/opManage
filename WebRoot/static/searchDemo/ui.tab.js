 ...
 */
 (function($) {
			...
			if($(_this.opts.tabList).length>0){
				$(_this.opts.tabList).each(function(index){
						$(this).bind(_this.opts.eventType,function(){
							...
							if($.inArray(index,_this.disableArr)==-1 && !$(this).hasClass(_this.opts.tabActiveClass)){ 
								//callback
								$(_this.opts.tabList).removeClass(_this.opts.tabActiveClass);
								$(this).addClass(_this.opts.tabActiveClass);
								_this._showContent(index);
							}else{
								$(this).removeClass(_this.opts.tabActiveClass);
								$(_this.opts.contentList+":visible").slideUp();
							}
						});
					});
			}
    ...
    $.fn.tab.defaults = {
	   ...
	   showType:"slide",										//显示方式，show 直接显示，fade渐变，slide滑动
	   showSpeed:200
    };
})(jQuery);