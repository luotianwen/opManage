...
//			};
		...
		FilesAdded: function(up, files) {
			makeThumb(files,0, function (imgsrc,file) {
				...
				$('#container').find("img").after("<div class='progress' style='width:200px'><div id='"+file.id+"' class='progress-bar' style='width: 0%;height: 15px;'></div></div>");
				set_upload_param(uploader, '', false);
			})
		},
		...
		FileUploaded: function(up, file, info) {
            if (info.status == 200)
            {
            	...
            	if(json.resultCode == 0){
            		...
            		$("#container").find("img").attr("data-src",host+get_uploaded_object_name(file.name));
            		$("#container").find("input").val($("#container").find("img").attr("data-src"));
            	}else{
