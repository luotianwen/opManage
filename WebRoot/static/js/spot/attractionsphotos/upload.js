	...
	init: {
		...
		FilesAdded: function(up, files) {
			makeThumb(files,0, function (imgsrc,file) {
				$('#photo').append(
					"<li>"+
						"<img src='"+imgsrc+"' name='"+file.name+"' style='width:200px;heigth:150px;' />"+
						"<div class='edit-img-box'>"+
						"	<a href='javascript:void(0)' id='delete' class='edit-del' title='删除'></a>"+
						"</div>"+
						"<div class='progress' style='width:100%'><div id='"+file.id+"' class='progress-bar' style='width: 0%;height: 15px;'></div></div>"+
					"</li>"
				);
            })
		},
		...
		FileUploaded: function(up, file, info) {
            if (info.status == 200)
            {
            	...
            	if(json.resultCode == 0){
            		...
            		$.post("attractionsphotos/saveAttractionsphotos.json",{sid:sid,pho:host+get_uploaded_object_name(file.name)},function(data){
            			if(data.RESPONSE_STATE=="200"){
            				layer.msg("上传成功。",{icon:1,time:1*1000,shade:0.3});
            				$("#"+file.id).closest("li").attr("data_id",data.id);
            				$("#"+file.id).closest("li").find(".progress").remove();
            				
            				var count = $("ul#photo").find("li").length;
            				parent.$("table tr td p[data-id='"+sid+"']").closest("tr").find("td#num").html(count);
            			}else{
            				layer.alert("上传图片异常，请稍后再试！",{icon:0,title:"失败提醒"})
            			}
            		})
            	}else{
